...
describe('getTodaySucceededJobs', () => {
  ...
  test('只返回今天 status=success 的 job 名(failed/partial 不算)', () => {
    const a = startJobRun(db, 'options');           finishJobRun(db, a, { status: 'success', recordsWritten: 1 });
    const b = startJobRun(db, 'vrp_inputs');         finishJobRun(db, b, { status: 'failed', error: 'x' });
    const c = startJobRun(db, 'vx_term_structure');  finishJobRun(db, c, { status: 'partial', recordsWritten: 1, error: 'y' });
  ...
  test('忽略往日的成功(只看本地日的今天)', () => {
    ...
    db.run(`INSERT INTO job_run (job_name, started_at, finished_at, status, records_written) VALUES (?, ?, ?, 'success', 1)`, ['options', yest, yest]);
