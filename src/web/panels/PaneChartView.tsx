  ...
  return (
    <div className="relative flex h-full w-full flex-col">
        ...
        {paneDefs.map(({ key, label: pl }) => {
          ...
          return (
            <div key={key} className="flex items-center gap-0.5 rounded border border-neutral-700 px-1 py-0.5 text-xs">
              <button onClick={() => move(key, -1)} disabled={pos === 0} title="上移" className={btn}>
                ↑
              </button>
              <button onClick={() => move(key, 1)} disabled={pos === order.length - 1} title="下移" className={btn}>
                ↓
              </button>
              <button
                onClick={() => toggle(key)}
      ...
      <div className="relative min-h-0 flex-1">
        ...
        {hovering &&
          order.map((key, i) => {
            ...
            return (
              <div
                key={key}
                className="pointer-events-none absolute left-2 z-10 text-xs leading-tight"
                style={{ top: (tops[i] ?? 0) + 2 }}
              >
                {def.series.map((sk) => {
                  const c = cells[sk];
        ...
        {badges &&
          order.map((key, i) => {
            ...
            return (
              <div
                key={key}
                className="pointer-events-none absolute right-14 z-10 text-xs text-neutral-400"
                style={{ top: (tops[i] ?? 0) + 2 }}
              >
                {b}
              </div>
        ...
        {error && (
          <p className="absolute left-2 top-2 text-xs text-red-400">
