...
const VIEW_DESC: Record<string, { title: string; desc: string }> = {
  ...
  real: {
    title: '实际走势',
    desc: [
      '定义:TIPS 实际收益率各期限的时间走势。名义 = 实际 + BEI 里的**实际**那一块。',
      '与「通胀走势」那格并排读 —— 长端在动时,先分清动的是通胀补偿还是实际利率:两者资产含义相反。',
      '  · 实际上行 + BEI 不动 = 真实融资成本上行(压估值,也可能是期限溢价在重估)',
      '  · BEI 上行 + 实际不动 = 通胀定价在动,不必然压估值',
      '',
      '⚠️ 实际 ≠ 期限溢价:实际里还含长期中性实际利率(R*)。要拆这一层看「长端分解」那格,',
      '但期限溢价模型只发到 10Y —— **30Y 段的成分拆解只有这条路**。',
      '⚠️ `名义 − 实际 ≈ BEI` 只是近似:三条曲线各自独立拉、各自可能缺日,别拿它当恒等式校验。',
    ].join('\n'),
  },
  ai_cds: {
    title: 'AI CDS',
...
const SPREAD_DESC: Record<string, string> = {
  ...
  real: [
    ...
    '',
    '⚠️ 与「通胀走势」的 30Y − 10Y BEI 对照读:实际这条走阔而 BEI 那条不动 = 归因落在久期风险补偿一侧。',
  ].join('\n'),
  ai_cds:
