    ...
    panes: [
      ...
      },
      {
        ...
        desc: [
          ...
          '',
          '⚠️ **它不是「买期权贵不贵」的判据** —— 它是期货曲线,回答的是 VXX / VIX 期货那类工具的',
          'carry 与恐慌结构。要判断该买多长期限的**期权**,看下面那格 VIX−3M(见其说明里的实测差异)。',
        ].join('\n'),
      },
      {
        key: 'vixSpotTerm',
        ...
        desc: [
          '定义:VIX(30 天)− VIX3M(3 个月),两者都是**恒定期限的即期隐含波动率**。',
          '**减号方向与上面 VX1−V3 一致**(近端 − 远端):绿 / 正 = backwardation,红 / 负 = contango。',
          '',
          '⭐ **它和 VX1−V3 不能互相替代**,实测过(2018-01-02~2026-08-07,2161 个交易日):',
          '**相关系数 0.876,而且 9.5%(205/2161)的交易日两者符号相反** —— 大约十一天里有一天,',
          '一个说 contango、另一个说 backwardation。差异有两个来源:',
          '  · **量纲不同**:VX3 是「三个月后那个 30 天波动率」的**远期**;VIX3M 是「现在三个月期权」的',
          '    **即期**隐含波动率。前者是远期的短期量,后者是即期的长期量。',
          '  · **VX1 在滚动**:第 1 近合约剩余期限每月从约 30 天走到 0,VX1−V3 自带月度锯齿;',
          '    恒定期限没有这个伪影。',
          '',
          '用法分工 —— 挑期限时看这格,不看 VX1−V3:',
          '  · 负(contango)= 远端的年化波动率**指数**高于近端;这是常态。',
          '  · ⭐ 转正(backwardation)= **近端比远端还高** = 市场在抢短期保护。同时意味着已经在事件里了 ——',
          '    保护该在转正之前布置,不是之后。',
          '',
          '⚠️ **它是指数,不是你要买的那张合约的报价**。VIX / VIX3M 由一篮子 SPX 行权价合成、是年化',
          '波动率**指数**,不等于任何单一合约的 IV,更不是权利金。所以这格只回答「曲线哪一段相对高/低」,',
          '**回答不了「那张 put 贵不贵」** —— 那要看行权价所在的波动率曲面位置与实际 bid/ask,',
          '去券商实时链上看。这格的职责到「该翻哪个到期日」为止。',
          '',
          '⚠️ 它是 **SPX 口径**。CBOE 不发纳指的期限结构(VXN9D / VXN3M / VOLQ 实测全部 403),',
          '所以对冲 QQQ 时只能读 SPX 曲线的**形状**、承受基差,没有更好的免费替代。',
        ].join('\n'),
      },
      {
        key: 'vix6m',
        ...
        desc: [
          '定义:SPX 6 个月恒定期限隐含波动率(CBOE VIX6M)。',
          '',
          '为什么单列这个期限:上面几格全是 30 天口径,而**保护很少只布置一个月** —— ',
          '要判断「现在这一段贵不贵」,得看你实际会持有的那个期限的水平**在它自己历史的什么位置**。',
          'P5/P95 带就是干这个的:曲线形状(上一格)告诉你远近谁高,这一格告诉你这一段的绝对水平如何。',
          '',
          '临界:同 VIX,低位不是安全 —— 长端被压到历史低分位 = 市场认为未来很久都没事。',
          '⚠️ **别拿它当择时信号**:长端波动率均值回归极慢,低分位可以待很久。',
          '⚠️ 同上一格:它是**年化波动率指数**,不是具体合约的 IV 或权利金。它回答「这一段在自身历史的',
          '什么位置」,不回答「那张期权多少钱」—— 后者去券商实时链上看。',
          PINNED_VOL,
        ].join('\n'),
      },
    ],
