...
15 家,五组。**组的顺序即资金流向**(§6.16 正向拆解链:FCF 增厚 → 扩 capex → 买设备),
**同组能横向比、跨组不能**。
...
| `memory` 存储 | MU SKHY SNDK | seller | **周期性**高毛利 |
...
⚠️ **存储三家按产品谱系分工**:MU = DRAM+NAND 全谱,SKHY = 重心 DRAM/HBM,SNDK = **纯 NAND**。
三条线并排才拆得开「HBM 拉动(AI 直接需求)」与「NAND 自身周期(供需缺口)」——
SNDK 里没有 HBM,它动 = NAND 侧真的在动。
...
META/AMZN/INTC
SNDK                  28~70                  29~71   ← 逐份实测 70/45/55/35/28/28。
                                                        70 = 分拆后首份 10-Q(一次性);
                                                        55 = FY2025 年报(年报本就慢);
                                                        10-Q 最近两份稳在 T+28
AMD                      39                    40
...
- **SNDK** ⚠️ **口径断点在分拆日、不在财年边界,而且不是一刀切、要按格分** —— 这条最容易推错,两层都要记:
  - 2025-02-21 从 WDC 分出来,分拆日落在 **2024-12-28~2025-03-28 那一季内部**,
    那一季自己就横跨两种口径。按财年划(「FY2025 及更早是 carve-out」)是错的。
  - **按格分的依据是 FY2025 10-K 自己印的分摊表**(accession `0002023554-25-000034`),
    它逐行列出母公司分摊落在哪:R&D 189 / SG&A 158 / 分拆费用 50 / 离职等 5(FY2025,百万美元;
    FY2024 合计 1,165、FY2023 合计 1,263)。**表里没有 cost of revenue 那一行**,于是:
    - **毛利率 / 营收:表里没有 COGS 行 → 不受这笔分摊影响,跨断点可读**。
      (只说可核的部分:MD&A 提到分摊还含 product sourcing / supply chain 这类天然属 COGS 的支持成本,
      但它没落进这张按行拆的表 —— 所以说「不受这笔分摊影响」,不说「毫无失真」。)
    - **FCF / 单季 FCF:分拆前不可比** —— 光 FY2024 就有 11.65 亿分摊走费用,还叠着母公司集中资金管理
      (FY2024 全年 OCF −3.09 亿美元)。干净单季自 2025-06-27 起、第一个干净 TTM 点 2026-04-03;
      **别把 FCF「转正」读成经营改善**,那是换口径不是变好。
  - 最近摆得很猛:TTM 毛利率 27.9%(2025-10-03)→ 56.0%(2026-04-03),**两个季度**;
    干净单季 26.2% → 29.8% → 50.9% → **78.4%**。同期**单季**营收同比 +251%(该格是单季同比,不是 TTM 同比)。
    ⚠️ 但**摆幅别和 MU 比高低**:MU 在库里 64 个点走过 87 个百分点,SNDK 只有 7 个点、区间 28 个 ——
    它是同一个 NAND 周期的第二个读数,不是更极端的那个。
  - ⚠️ **2025-10-03 那根同比柱多一周**:FY2026 是 53 周财年,多出的一周在 Q1(98 天对去年同季 91 天)。
    **按周均线性折算,+22.6% 里约 8.8 个百分点来自这一周**(本季按 13/14 折成 2,143 再比去年同季 1,883,
    同比只剩 13.8%)。注意分母是**去年同季**,不是本季 —— 拿 98/91−1 = 7.7% 当百分点是错的。
  - **capex 那格读不出供给侧信息**:产能在与铠侠的合资厂 Flash Ventures 里走权益法,
    自己账上 capex TTM 只有 1.78~2.04 亿(对 132 亿 TTM 营收),别和 MU / TSM 的扩产并排看。
...
com.mtv.sec   天天 13:00 / 19:00   → src/server/jobs/aiChainFundamentals.ts
   ├── sec_fundamentals   companyfacts,12 家
