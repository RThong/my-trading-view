...
cp .env.example .env
$EDITOR .env                    # moomoo OpenD host/port/key + FRED_API_KEY / EIA_API_KEY (both free)
...
need `FRED_API_KEY` and — for the refinery/inventory tab — `EIA_API_KEY` (both free, email
signup); CBOE/CNN need nothing. See [AGENTS.md](AGENTS.md) for gotchas.
